...
public class VendedorController {
    ...
    void mostrarDialogoRegistroAction() {
        ...
        TextField fCiudad = new TextField();    fCiudad.setPromptText("Ej: Bogotá");
        ...
        TextField fDesc = new TextField();      fDesc.setPromptText("Descripción breve");
        ComboBox<TipoInmueble> fTipo = new ComboBox<>();
        fTipo.setItems(FXCollections.observableArrayList(TipoInmueble.values()));

        grid.add(new Label("Dirección:"),   0, 0); grid.add(fDireccion, 1, 0);
        grid.add(new Label("Ciudad:"),      0, 1); grid.add(fCiudad,    1, 1);
        grid.add(new Label("Área (m²):"),   0, 2); grid.add(fArea,      1, 2);
        ...
        dialog.showAndWait().ifPresent(result -> {
            ...
            try {
                ...
                mostrarAlerta("Área y precio deben ser números válidos.");
            }
        });
    }
    ...
    void publicarInmuebleAction() {
        ...
        if (descripcion.isEmpty()) {
            lblMsgPublicacion.setText("Escribe una descripción para la publicación.");
            return;
        }
    ...
    private void procesarCierre(TipoOperacion tipo) {
        ...
        lblMsgOferta.setText("Operación de " + tipo.name() + " completada exitosamente.");
        cargarDatos();
        cargarOfertasAction();
    }
